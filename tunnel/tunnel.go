...
func Start(localIP string, localPort int) (*Tunnel, error) {
	cmd := exec.Command("ssh",
		...
		"-R", fmt.Sprintf("80:%s:%d", localIP, localPort),
		"nokey@localhost.run",
	)

	// localhost.run writes the URL to stdout
	...
	// Scan stdout line by line until we find the URL
	...
	go func() {
		...
		errCh <- fmt.Errorf("ssh process exited before URL was found")
	}()
