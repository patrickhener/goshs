...
function deleteSelected() {
  ...
  Promise.all(
    ...
    .then(() => {
      toast("Deleted " + names.length + " item(s)", "success");
...
function showQR() {
  ...
  // simple QR via canvas - basic URL display fallback (real implementation uses qrcode.js)
  ...
  ctx.fillText("QR: " + url.slice(0, 28), 10, 100);
  openModal("qr-modal");
}
